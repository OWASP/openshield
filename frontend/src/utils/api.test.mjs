...
function loadApiModule(options = {}) {
  const { fetchImpl, timers, token = null } = typeof options === 'function' ? { fetchImpl: options } : options;
  ...
  source += `\nreturn {
    ...
    normalizeScore, normalizeComplianceFramework,
  };`;
...
test('public API methods accept operation-specific timeout overrides', async () => {
  ...
  assert.deepEqual(await api.getScore({ timeoutMs: 1250 }), { status: 'OK', score: 90, max_score: 100 });
...
function check(description, fn) { test(description, fn); }
function checkAsync(description, fn) { test(description, fn); }
...
checkAsync('api.getScore() surfaces NO_SCAN_DATA from the real response shape without coercion', async () => {
  const { api } = loadApiModule(() =>
...
checkAsync('api.getScore() surfaces a real evaluated score unchanged', async () => {
